...
const NearbyLandmarks = () => {
  ...
  return (
    <>
      <section className="py-16 bg-gradient-to-br from-icon/5 to-bronze/5">
            ...
            <div className="grid md:grid-cols-2 gap-8 items-start">
              ...
              <div className="space-y-3">
                {landmarks.map((landmark, index) => (
                  <div key={index} className="flex items-center gap-4 p-4 rounded-xl bg-card border border-border shadow-soft animate-fade-in-up" style={{
                    animationDelay: `${index * 60}ms`
                  }}>
                    <div className="flex-shrink-0 p-3 rounded-xl bg-icon/20">
                      <landmark.icon className="h-5 w-5 text-icon-dark" />
                    </div>
